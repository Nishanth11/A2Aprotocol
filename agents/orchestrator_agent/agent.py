...
DO NOT include any text before or after the JSON. Output ONLY the JSON object."""
    ...
    def _extract_json_from_text(self, text: str) -> Optional[Dict[str, Any]]:
        ...
        # 3. Bracket-matching: find the outermost {...} — handles nested objects
        start = text.find('{')
        while start != -1:
            depth = 0
                ...
                if text[i] == '{':
                    ...
                    if depth == 0:
                        try:
                            ...
                            if isinstance(data, dict) and 'decision' in data:
                                logger.info("Successfully extracted JSON via bracket matching")
                                return data
                        except Exception:
                            pass
                        break
    ...
    async def invoke(self, query: str, session_id: str = None) -> str:
        ...
        try:
            # Step 1: Call both child agents directly — no tool-calling LLM needed
            equipment_result = await asyncio.wait_for(
                self._delegate_task(
            ...
            material_result = await asyncio.wait_for(
                self._delegate_task("MaterialCalculatingAgent", query, session_id),
                timeout=self.child_agent_timeout,
            )

            logger.info(f"Equipment result: {equipment_result[:300]}")
            logger.info(f"Material result: {material_result[:300]}")

            # Step 2: Ask LLM to synthesise both results into the required JSON
            from langchain_core.messages import HumanMessage
            synthesis_prompt = f"""You are an Orchestrator Agent for a Batch Plant.

User query: {query}

Equipment data (from EquipmentMonitoringAgent):
{equipment_result}

Material requirements (from MaterialCalculatingAgent):
{material_result}

Based on the data above, decide if production is feasible.
...
{{
    ...
    "tools_used": ["MaterialCalculatingAgent", "EquipmentMonitoringAgent"]
            ...
            raw = await loop.run_in_executor(None, self.llm.invoke, [HumanMessage(content=synthesis_prompt)])
            text_output = raw.content if hasattr(raw, "content") else str(raw)
            logger.info(f"LLM synthesis output: {text_output[:500]}")

            json_data = self._extract_json_from_text(text_output)
            if json_data:
                try:
                    parsed = ProductionAssistantResponse(**json_data)
                    logger.info("Successfully created valid response")
                    return parsed.model_dump_json()
                except ValidationError as e:
                    logger.error(f"Validation error: {e}")
            else:
                logger.error(f"Could not extract JSON from LLM output: {text_output}")
        ...
        error_response = ProductionAssistantResponse(
            decision="Error",
            reasoning="Failed to process the request.",
