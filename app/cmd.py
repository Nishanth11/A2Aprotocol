...
# the EquipmentMonitoringAgent running on an A2A server.
#
# It sends simple text messages (like "What is the current material availability and machines states") to the agent,
...
# -----------------------------------------------------------------------------
@click.command()
@click.option("--agent", default="http://localhost:40002", help="Base URL of the A2A agent server")
...
async def cli(agent: str, session: str, history: bool):
    ...
    Args:
        agent (str): The base URL of the A2A agent server (e.g., http://localhost:40002)
    ...
    while True:
        ...
        prompt = await click.prompt("\nHow can I help you? (type ':q' or 'quit' to exit)")
